...
for (test_season in c("2015","2016","2017")){
  ...
  for (test_week in c(seq(40,52),seq(end_week))){
    ...
    for (reporting_delay_adjustment in c(TRUE,FALSE,"TRUTH","LAGDF")){
      ...
      if (reporting_delay_adjustment == TRUE){
      ...
      mean_forecast_trajectory <- forecast(arima_updated_fit,h=ph)$mean
